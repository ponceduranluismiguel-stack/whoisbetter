    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #0a1740 0%, #02081f 100%)', paddingBottom: '80px' }}>
      ...
      <div style={{ overflowX: 'auto', padding: '12px 16px 0' }}>
        <div style={{ display: 'flex', gap: '8px', paddingBottom: '4px' }}>
          {competiciones.map(c => (
            <button key={c.id} onClick={() => setCompActiva(c.id)} style={{
              display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0,
              padding: '6px 12px', borderRadius: '20px', border: 'none', cursor: 'pointer',
              background: compActiva === c.id ? '#ffd400' : 'rgba(255,255,255,0.08)',
              color: compActiva === c.id ? '#0a1740' : '#fff',
            }}>
              <div style={{ width: '18px', height: '18px', background: compActiva === c.id ? 'rgba(0,0,0,0.1)' : 'rgba(255,255,255,0.9)', borderRadius: '4px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <img src={`${API_URL}/img/football/leagues/${c.logoId}.png`} alt={c.nombre}
      ...
      <div ref={jornadasRef} style={{
        overflowX: 'auto', padding: '14px 16px 10px',
          ...
          Array.from({ length: maxJornada }, (_, i) => i + 1).map(n => {
            ...
            return (
              <button
                key={n}
                data-jornada={n}
                onClick={() => setJornada(n)}
                style={{
                  width: '34px', height: '34px', borderRadius: '50%', flexShrink: 0,
                  border: '2px solid #ffd400',
                  background: activa ? '#ffd400' : 'transparent',
                  color: activa ? '#0a1740' : '#ffd400',
                  fontFamily: 'Anton, sans-serif',
                  fontSize: '0.7rem',
                  cursor: 'pointer',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  transition: 'all 0.15s',
                  boxShadow: activa ? '0 2px 12px rgba(255,212,0,0.4)' : 'none',
                }}
              >
                {n}
              </button>
      ...
      <div style={{ padding: '0 16px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <button onClick={() => jornada > 1 && setJornada(jornada - 1)} disabled={!jornada || jornada <= 1} style={{
          width: '36px', height: '36px', borderRadius: '50%',
          background: !jornada || jornada <= 1 ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.1)',
          border: 'none', color: !jornada || jornada <= 1 ? 'rgba(255,255,255,0.2)' : '#fff',
          cursor: !jornada || jornada <= 1 ? 'default' : 'pointer',
        ...
        <button onClick={() => jornada < maxJornada && setJornada(jornada + 1)} disabled={!jornada || jornada >= maxJornada} style={{
          width: '36px', height: '36px', borderRadius: '50%',
          background: !jornada || jornada >= maxJornada ? 'rgba(255,255,255,0.04)' : 'rgba(255,255,255,0.1)',
          border: 'none', color: !jornada || jornada >= maxJornada ? 'rgba(255,255,255,0.2)' : '#fff',
          cursor: !jornada || jornada >= maxJornada ? 'default' : 'pointer',
          fontSize: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'center',
        }}>→</button>
      </div>
      ...
      <div style={{ padding: '0 16px' }}>
        {cargando || cargandoJornadas ? (
          <div style={{ padding: '40px', textAlign: 'center', color: '#ffd400', fontFamily: 'Anton, sans-serif', fontSize: '0.9rem', letterSpacing: '1px' }}>
            CARGANDO...
          </div>
        ) : !partidos || Object.keys(partidos).length === 0 ? (
          <div style={{ padding: '40px', textAlign: 'center', color: '#7a8aa8', fontFamily: 'Anton, sans-serif', fontSize: '0.8rem', letterSpacing: '1px' }}>
            ...
            <div key={fecha} style={{ marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                {fixtures.map((f, i) => {
                  ...
                  return (
                    <div key={i} style={{
                      background: enJuego ? 'rgba(34,197,94,0.08)' : 'rgba(255,255,255,0.03)',
                      borderRadius: '12px',
                      boxShadow: enJuego ? 'inset 0 0 0 1px rgba(34,197,94,0.3)' : 'inset 0 0 0 1px rgba(255,255,255,0.05)',
                      padding: '12px 14px',
                      display: 'grid',
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px', justifyContent: 'flex-end' }}>
                        <div style={{ fontFamily: 'Anton, sans-serif', fontSize: '0.8rem', color: '#fff', textTransform: 'uppercase', textAlign: 'right', letterSpacing: '-0.2px', lineHeight: 1.2 }}>
                          {f.teams.home.name}
                        </div>
                        <div style={{ width: '28px', height: '28px', background: 'rgba(255,255,255,0.95)', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <img src={`${API_URL}/img/${f.teams.home.logo.replace('https://media.api-sports.io/', '')}`} alt={f.teams.home.name}
                      ...
                      <div style={{ textAlign: 'center' }}>
                        {finalizado || enJuego ? (
                          ...
                          <div style={{ fontFamily: 'Anton, sans-serif', fontSize: '0.9rem', color: '#ffd400' }}>
                            {formatearHora(f.fixture.date)}
                          </div>
                        )}
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{ width: '28px', height: '28px', background: 'rgba(255,255,255,0.95)', borderRadius: '6px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <img src={`${API_URL}/img/${f.teams.away.logo.replace('https://media.api-sports.io/', '')}`} alt={f.teams.away.name}
                            style={{ width: '22px', height: '22px', objectFit: 'contain' }}
                            onError={e => { e.target.style.display = 'none' }} />
                        </div>
                        <div style={{ fontFamily: 'Anton, sans-serif', fontSize: '0.8rem', color: '#fff', textTransform: 'uppercase', letterSpacing: '-0.2px', lineHeight: 1.2 }}>
                          {f.teams.away.name}
                        </div>
                      </div>
                    </div>
